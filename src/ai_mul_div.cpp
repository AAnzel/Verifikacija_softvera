...
private:
	...
	// Utils:
	// load instrukcija se koristi za citanje iz memorije
	// %val = load i32, i32* %ptr  
	// ucitava u val max 4 bajta a cita iz ptr 
	IntervalOfConf* parseLoad(Value* vred){
		...
		*/
		IntervalOfConf* i = new IntervalOfConf(inst->getName()); // interval u koji loadujemo			
		
		std::string name = (inst->getOperand(0))->getName(); // ime iz kog loadujemo
		IntervalOfConf* j = findByName(name); // nalazimo interval iz kog se loaduje prema imenu
		if(j!=nullptr){
			if(j->is_bottom()) // ako mu donja granica nije -inf
				i->set_bottom(j->get_bottom()); // postavljamo donju granicu i na donju granicu j
			if(j->is_top()) // ako mu gornja granica nije +inf
				i->set_top(j->get_top()); // postavljamo gornju granicu i na gornju granicu j
		}else{
	...
	// store instrukcija se koristi za upis u memoriju
	// u ovom primeru upisujemo 7 u tmp1
	IntervalOfConf* parseStore(Value* vred){
		...
		if(v1->getName()==""){
		  ...
		  Instruction *_inst2 = dyn_cast<Instruction>(v2);
			...
			{
				i->set_value(konst_int->getValue()); // interval ima istu gornju i donju granicu i predstavlja samo jednu tacku, tj samo jedan konst int
			}else{
				outs() << "STORE ERROR!\n";
			}
		}else{
			// store from virtual register
			std::string name = v1->getName();  // tmp1 npr
			IntervalOfConf* j = findByName(name); // nadjemo interval sa tim imenom 

			if(j!=nullptr){ //prekopiramo njegovu gornju i donju granicu
				if(j->is_bottom())
	...
	// alloca instrukcija alocira memoriju na okviru steka funkcije koja se trenutno izvrsava; memorija se alocira, vraca se pokazivac
	...
	IntervalOfConf* parseRet(Value* vred){
		...
		std::string name = (inst->getOperand(0))->getName(); // tmp7
		IntervalOfConf* j = findByName(name); // nadjemo interval od tmp7
		if(i!=nullptr){ // povratni interval se postavlja na interval od tmp7
			if(j->is_bottom())
				i->set_bottom(j->get_bottom());
			if(j->is_top())
				i->set_top(j->get_top());
	...
	// Add instrukcija vraca zbir operanada
	// <result> = add i32 4, %var  daje i32:result = 4 + %var -- sabiranje sa konstantom
	...
	IntervalOfConf* parseMul(Value* vred){
		...
		IntervalOfConf* j = findByName(v1->getName());
		IntervalOfConf* k = findByName(v2->getName());
		if(j!=nullptr && k!=nullptr){  // j * k: [2, 4] * [3, 10] = [6, 40]
			if(j->is_bottom() && k->is_bottom())
				i->set_bottom(j->get_bottom()*k->get_bottom());
			if(j->is_top() && k->is_top())
				i->set_top(j->get_top()*k->get_top());
	...
	IntervalOfConf* parseIcmp(Value* vred){
		...
		if(auto* konst_int = dyn_cast<ConstantInt>(v2)){
			if(cast<Instruction>(v1)->getOpcode()==30){ // 30 je instrukcija load
				// %tmp < 5
				...
				l1 = findByName(v1->getName()); //tmp2
				l2 = findByName((cast<Instruction>(v1)->getOperand(0))->getName());  // tmp1 
